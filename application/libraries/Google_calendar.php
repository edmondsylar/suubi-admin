                ...
                if ($user_id && get_setting('user_' . $user_id . '_oauth_access_token') && get_setting('user_' . $user_id . '_google_calendar_authorized')) {
                    ...
                    if ($calendar_ids) {
                        ...
                        if (count($calendar_ids_array)) {
                            $user_email = $this->ci->Users_model->get_one($user_id)->email;

                            foreach ($calendar_ids_array as $calendar_id) {
                                if ($calendar_id != $user_email) { //user's private calender id and his email is same
        ...
        if ($events && count($events)) {
            foreach ($events as $event) {
                ...
                $data = array(
                    ...
                    "client_id" => 0,
                    "google_event_id" => $google_event_id,
                    "labels" => lang("google_calendar_event"),
